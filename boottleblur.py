...
from darkpoold.lib import config
...
class XCPManager(Tk):
    def __init__(self):
        ...
        self.defaultvalues = {
            ...
            "craved-rpc-connect": config.BITCOIND_RPC_CONNECT,
            "craved-rpc-port": config.BITCOIND_RPC_PORT,
            "craved-rpc-user": config.BITCOIND_RPC_USER,
            "craved-rpc-password": config.BITCOIND_RPC_PASSWORD
        }

        self.allkeys = [
            "gui-host", "gui-port", "gui-user", "gui-password", 
            "craved-rpc-connect", "craved-rpc-port", "craved-rpc-user", "craved-rpc-password"
        ]
        self.configpath = os.path.join(config.DATA_DIR, 'darkpoold.conf')       
    ...
    def start_party(self):     
        if (not check_config()):
            ...
            if self.ws_subprocess is None:
                ...
                try:
                    self.ws_subprocess = subprocess.Popen([self.python_path, 'darkpoolws.py'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False)
                except Exception as e:
                    print(e)
